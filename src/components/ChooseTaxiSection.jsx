...
function VehicleCard({ v }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col items-center relative overflow-hidden group">
      <div className="pt-8 sm:pt-10 px-4 sm:px-6 flex-1 flex flex-col items-center w-full">
        <div className="w-full mb-5 sm:mb-6 relative rounded-t-2xl overflow-hidden bg-[#f7f7f7] flex items-center justify-center">
          <img
            src={v.image}
            alt={`${v.name} ${v.model}`}
            className="w-full h-40 sm:h-56 object-cover rounded-t-2xl transition-transform duration-500 group-hover:scale-[1.05]"
            loading="lazy"
          />
        </div>
        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-brandBlack text-white grid place-items-center text-2xl sm:text-3xl mb-5 sm:mb-6">
          <i className="fa-solid fa-taxi" aria-hidden="true" />
        </div>
        <h3 className="font-semibold text-base sm:text-lg tracking-tight text-center mb-2">
          {v.name} {v.model} Model
        </h3>
        <ul className="flex flex-wrap justify-center gap-x-3 sm:gap-x-4 gap-y-1 text-[10px] sm:text-[11px] uppercase font-semibold tracking-wider text-brandBlack/60 mb-5 sm:mb-6">
...
export default function ChooseTaxiSection() {
  ...
  return (
    <section
      id="fleet"
      className="bg-white text-brandBlack py-20 sm:py-28 px-5 sm:px-6 lg:px-10"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <h2 className="text-3xl sm:text-5xl font-extrabold tracking-tight text-center mb-10 sm:mb-12">
          Choose taxi
        </h2>
            ...
            return (
              <button
                ...
                className={classNames(
                  "px-5 sm:px-7 py-3 sm:py-4 rounded-xl text-[10px] sm:text-[11px] font-semibold tracking-wide transition-all",
                  active
